...
public:
	...
	enum AnimMontages
	{
		EHit, EAtack, EParry, EDefense, EDash, EDie, EExecution, ESpecialAtack, ECreateClone, EGrab, EComboAtack, ETeleport, EComboAtack2
	};

protected:
	enum States
	{
		sIdleMov, sHit, sStartAttack, sAttack, sAttackEnd, sAttackRoll, sDefense, sStartParry, sParry, sEndParry, sGrab
	};
...
public:	
	...
	UFUNCTION()
	void ChangeStamina(const float Val);

	UFUNCTION()
	virtual int32 TakeHit(const int32 Damage, const int32 Atack);
	...
	UFUNCTION()
	void LoseHealth(const float Val);

	UFUNCTION()
	virtual void ChangeBlackboarValue(const FName Description, bool Val) {};

	UFUNCTION()
	virtual void ConectAttack(const FName Description) {};
	...
	UFUNCTION()
	virtual void StartAttack(const FName IdAttack) {};
	...
	UFUNCTION()
	bool IsUnstoppableAttack() { return bUnstoppableAttack; }
